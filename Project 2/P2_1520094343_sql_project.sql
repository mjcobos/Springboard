...
/*A1: Tennis Court 1, Tennis Court 2, Message Room 1, Message Room 2, Squash Court.
...
SELECT f.name AS court_name, CONCAT(m.firstname, ' ', m.surname) as member_name,
						     CASE WHEN m.memid = 0
                                  THEN f.guestcost
                                  ELSE f.membercost END AS facility_cost
  FROM Bookings b
  JOIN Facilities f
    ON f.facid = b.facid
  JOIN Members m
